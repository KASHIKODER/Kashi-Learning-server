...
export const getNotifications = CatchAsyncError(async (req: Request , res: Response , next: NextFunction)=>{
    try{
        const notifications = await NotifactionModel.find().sort({createdAt : -1});
...
export  const updateNotification = CatchAsyncError(
    async ( req: Request , res: Response , next: NextFunction) => {
        try{
            ...
            if(!notification){
                return next(new ErrorHandler("Notifaction not found", 404));
            }else{
             notification.status
...
cron.schedule("0 0 0 * * *", async() => {
    ...
    await NotifactionModel.deleteMany({status:"read",createdAt: {$lt: thirtyDaysAgo}});
